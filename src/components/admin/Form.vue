<template>
  <form @submit.prevent="saveDoc">
    <div class="row">
      <div class="col-6">
        <input type="text" v-model.trim="doc.title" class="form-control form-control-sm" placeholder="Заголовок"
          maxlength="70" required />
      </div>
      <div class="col-6">
        <input v-if="collection === 'menus' || collection === 'napravs' || collection === 'prices'" type="text"
          v-model.trim="doc.alias" class="form-control form-control-sm" placeholder="Алиас" required />
      </div>
    </div>
    ...
    <div class="row mt-3">
      <div class="col-6">
        <div class="form-group row">
          ...
          <div class="col-4">
            <input type="number" id="position" min="0" max="42" step="1" v-model.number="doc.position" required
    ...
    <div v-if="collection != 'menus' || collection != 'contacts'" class="row mt-3">
      <div class="col-6 text-left">
        <div class="form-group">
          ...
          <input type="file" class="form-control-file" id="img" accept="image/png, image/jpeg"
            :disabled="doc.id ? false : true" @change="uploadImage" />
        </div>
      </div>
      <div v-if="doc.id" class="col-6">
        <img v-if="doc.img.name" :src="doc.img.url" alt="Изображение" class height="76" />
        <button v-if="doc.img.name" class="btn btn-sm btn-light position-relative load-image" type="button"
          @click="removeImage">Удалить</button>
      </div>
    </div>
      ...
      <div class="col-12">
        <div class="form-group text-left">
          ...
          <div class="input-group input-group-sm mb-3">
            <input type="text" class="form-control" placeholder="Здесь появится ссылка" id="imgSrc"
              v-model="doc.img.url" />
            <div class="input-group-append">
              <button class="btn btn-outline-secondary" type="button" @click="copyToBuffer">Копировать</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="row mt-3">
      <div class="col-12 col-sm-6 col-md-4">
        <transition name="fade" mode="out-in">
          <button @click="removeDoc" type="button" class="btn btn-sm btn-block btn-outline-danger"
            :disabled="doc.id ? false : true">Удалить</button>
        </transition>
      </div>
